"""
ac6 curtains scene width - pulse width dependence

source regions at various distances from ac6 and multiple Ls
Different pulse width at each L
...
pulse_widths = [0.05, 0.1, 0.2] # pulse width, seconds
...
settings = {'Ls0':L0s,'r0':r0,'E0':E0,'J0':J0,'pulse_period':pulse_period,
            'pluse_widths':pulse_widths,'t0':t0,'time_shift':time_shift,
            'oversample':oversample,'PHI0_DEG':PHI0_DEG,
            'debug':dipole_tracer.debug}
with open('scene-width-settings.json','w') as fid:
    json.dump(settings,fid,indent=1)


for (iphi0,phi0_deg) in enumerate(PHI0_DEG):
    phi0 = np.radians(phi0_deg)
    regions = [SourceRegion(L0,phi0,r0,E0,J0,pulse_period,pulse_width,t0,color=colors[iphi0]) for (L0,pulse_width) in zip(L0s,pulse_widths)]
...
plt.savefig('figures/scene-width-scene')
...
for (pulse_width,L0,it) in zip(pulse_widths,L0s,it0):
    plt.text(ac6a['t'][it],-0.002,'$\Delta$T$_s$=%g s' % (pulse_width),ha='center',va='top')
...
leg = plt.legend(fontsize='small',loc='upper right')
plt.savefig('figures/scene-width-rates')

plt.xlim([ac6a['t'][it0[0]]-1,ac6a['t'][it0[-1]]+1])
leg.remove()
plt.savefig('figures/scene-width-rates-noleg')
